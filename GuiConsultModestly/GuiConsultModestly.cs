...
namespace GuiConsultModestly
{
    ...
    public static class Main
    {
        ...
        static string title = "鬼的虚心请教 1.0.0";
        ...
        public static class SkillBattleSystem_AnQusetion_Patch
        {
            ...
            static string[] SetSkillBatterGains(bool isActor)
            {
                ...
                if (num2 != 10 && (stage==1 || num >= 5))
                {
                    ...
                    {
                        result[0] = "一知半解";
                        result[1] = selfName + "听了" + opponentName + "讲解受益良多，但" + book_name + "颇有难度，心中仍有多处不解。";
                        value = 2;
                    }
                    ...
                    if (value > 0)
                    {
                        value = UnityEngine.Random.Range(0, value + 1);
                        value = value == 0 ? 0 : SkillUpLevel(_skillId, value);
                        if (value > 0)
                        {
                            var seven = ActorMenu.instance.GetActorResources(_actorId);
                            int rand = UnityEngine.Random.Range(10, 110) + seven[1]; // 七元赋性: 0 = 细腻  1 = 聪颖  2 = 水性  3 = 勇壮  4 = 坚毅  5 = 冷静  6 = 机缘
            ...
            static int SkillUpLevel(int skillId, int value)
            {
