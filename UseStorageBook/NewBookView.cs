...
namespace Sth4nothing.UseStorageBook
{
    ...
    //     {
    //         if (pool.ContainsKey(key) && pool[key] != null && pool[key].Count > 0)
    //         {
    //             return pool[key].Dequeue();
    //         }
    //         if (prefabs.ContainsKey(key))
    //         {
    //             return UnityEngine.Object.Instantiate(prefabs[key], Vector3.zero, Quaternion.identity);
    //         }
    //         return null;
    //     }
    //     public void Recyle(string key, GameObject obj)
    //     {
    //         if (!pool.ContainsKey(key) || pool[key] == null)
    //         {
    //             pool[key] = new Queue<GameObject>();
    //         }
    //         pool[key].Enqueue(obj);
    //         while (size > 0 && pool[key].Count > size)
    //         {
    //             var tmp = pool[key].Dequeue();
